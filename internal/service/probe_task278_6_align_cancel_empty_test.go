...
func TestAlignCancelKeepsPriorAttributions(t *testing.T) {
	svc := newTestService(t)
	b, _ := mustBatch(t, svc, "ALIGN-CANCEL", 6)
	first, err := svc.Align(context.Background(), b.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(first.Attributions) == 0 {
		t.Fatal("setup align empty")
	}
	...
	_, _ = svc.Align(ctx, b.ID)
	left, err := svc.ListAttributions(b.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(left) == 0 {
		t.Fatal("cancel committed empty attributions")
	}
	if len(left) != len(first.Attributions) {
		t.Fatalf("want %d attributions after cancel, got %d", len(first.Attributions), len(left))
	}
}
