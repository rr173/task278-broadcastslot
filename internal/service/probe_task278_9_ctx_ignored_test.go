...
func TestCorrectRespectsCanceledContext(t *testing.T) {
	svc := newTestService(t)
	b := model.EvidenceBatch{Code: "CTX-IGN", Station: "S", AirDate: "1952-01-01", Timezone: "UTC"}
	if err := svc.CreateBatch(&b); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.AddEntry(b.ID, "e0", "CS", 1000, 2000, "p", "TX"); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		t.Fatal("canceled correct must fail")
	}
	...
	if len(rows) != 0 {
		t.Fatalf("canceled correct wrote %d rows", len(rows))
	}
}
